...
### Generator layers
def FCN_MASK(input, reuse, norm='instance', is_training=True, n=6):
  depth = input.get_shape()[3]
  for i in range(1,n+1):
    output = Rk_FCN_MASK(input, depth, reuse, norm, is_training, 'R{}_{}_FCN_MASK'.format(depth, i))
    input = output
...
def Rk_FCN_MASK(input, k,  reuse=False, norm='instance', is_training=True, name=None):
