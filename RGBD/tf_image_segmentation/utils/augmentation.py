...
def flip_randomly_left_right_image_with_annotation(image_tensor, annotation_tensor,depth_tensor,image_tensor2,annotation_tensor2,depth_tensor1):
    ...
    randomly_flipped_img2 = control_flow_ops.cond(pred=tf.equal(random_var, 0),
                                                 fn1=lambda: tf.image.flip_left_right(image_tensor2),
                                                 fn2=lambda: image_tensor2)
    ...
    randomly_flipped_annotation2 = control_flow_ops.cond(pred=tf.equal(random_var, 0),
                                                        fn1=lambda: tf.image.flip_left_right(annotation_tensor2),
                                                        fn2=lambda: annotation_tensor2)
    randomly_flipped_depth1 = control_flow_ops.cond(pred=tf.equal(random_var, 0),
                                                        fn1=lambda: tf.image.flip_left_right(depth_tensor1),
                                                        fn2=lambda: depth_tensor1)

    randomly_flipped_depth = control_flow_ops.cond(pred=tf.equal(random_var, 0),
                                                        fn1=lambda: tf.image.flip_left_right(depth_tensor),
                                                        fn2=lambda: depth_tensor)
    
    return randomly_flipped_img, randomly_flipped_annotation,randomly_flipped_depth,randomly_flipped_img2, randomly_flipped_annotation2,randomly_flipped_depth1

def flip_randomly_left_right_image_with_annotation_true(image_tensor, annotation_tensor):
    """Accepts image tensor and annotation tensor and returns randomly flipped tensors of both.
    The function performs random flip of image and annotation tensors with probability of 1/2
    The flip is performed or not performed for image and annotation consistently, so that
    annotation matches the image.
...
def scale_randomly_image_with_annotation_with_fixed_size_output(img_tensor,
								...
								depth_tensor,
                                                                ...
                                                                mask_out_number=255,img_tensor2=None,annotation_tensor2 = None,depth_tensor1= None):
    ...
    img_batched2 = tf.expand_dims(img_tensor2, 0)
    annotation_batched = tf.expand_dims(annotation_tensor, 0)
    annotation_batched2 = tf.expand_dims(annotation_tensor2, 0)

    depth_batched = tf.expand_dims(depth_tensor, 0)
    depth_batched1 = tf.expand_dims(depth_tensor1, 0)
    ...
    annotation_batched2 = tf.to_int32(annotation_batched2)
    ...
    resized_img2 = tf.image.resize_nearest_neighbor( img_batched2, scaled_input_shape )
    resized_annotation = tf.image.resize_nearest_neighbor( annotation_batched, scaled_input_shape )
    resized_annotation2 = tf.image.resize_nearest_neighbor( annotation_batched2, scaled_input_shape )
    resized_depth = tf.image.resize_nearest_neighbor( depth_batched, scaled_input_shape )
    resized_depth1 = tf.image.resize_nearest_neighbor( depth_batched1, scaled_input_shape )

    resized_img = tf.squeeze(resized_img, axis=0)
    resized_img2 = tf.squeeze(resized_img2, axis=0)
    resized_annotation = tf.squeeze(resized_annotation, axis=0)
    resized_annotation2 = tf.squeeze(resized_annotation2, axis=0)
    resized_depth = tf.squeeze(resized_depth, axis=0)
    resized_depth1 = tf.squeeze(resized_depth1, axis=0)
    ...
    annotation_shifted_classes2 = resized_annotation2 + 1

    cropped_padded_img = tf.image.resize_image_with_crop_or_pad( resized_img, output_shape[0], output_shape[1] )
    cropped_padded_img2 = tf.image.resize_image_with_crop_or_pad( resized_img2, output_shape[0], output_shape[1] )
    cropped_padded_annotation = tf.image.resize_image_with_crop_or_pad(annotation_shifted_classes,
                                                                       output_shape[0],
                                                                       output_shape[1])
    cropped_padded_annotation2 = tf.image.resize_image_with_crop_or_pad(annotation_shifted_classes2,
                                                                       output_shape[0],
                                                                       output_shape[1])
    cropped_padded_depth = tf.image.resize_image_with_crop_or_pad( resized_depth, output_shape[0], output_shape[1] )
    cropped_padded_depth1 = tf.image.resize_image_with_crop_or_pad( resized_depth1, output_shape[0], output_shape[1] )
    ...
    annotation_additional_mask_out2 = tf.to_int32(tf.equal(cropped_padded_annotation2, 0)) * (mask_out_number+1)
    cropped_padded_annotation = cropped_padded_annotation + annotation_additional_mask_out - 1
    cropped_padded_annotation2 = cropped_padded_annotation2 + annotation_additional_mask_out2 - 1
    return cropped_padded_img, cropped_padded_annotation,cropped_padded_depth,cropped_padded_img2,cropped_padded_annotation2,cropped_padded_depth1
def scale_randomly_image_with_annotation_with_fixed_size_output_true(img_tensor,
                                                                annotation_tensor,
                                                                output_shape,
                                                                min_relative_random_scale_change=0.9,
                                                                max_realtive_random_scale_change=1.1,
